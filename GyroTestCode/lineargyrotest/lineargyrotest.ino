...
#define to_standing_pin 50
float setpoint = 3.75;
const int seconds = 5; // time delay
...
void setup() 
{
  ...
  if (!mpu.begin()) 
  {
    Serial.println("Failure to get Gyro");
...
//    }
  ...
  {
  digitalWrite(LED_BUILTIN,LOW); //led low means gyro is connected properly
  ...
  //enable the to standing interupt
  
  pinMode(to_standing_pin, INPUT);
  Serial.println("Begin Main Functionality");
}

void loop() 
{
  
 Serial.println(digitalRead(to_standing_pin));
  while(singleLoop)
  {
    Serial.print("Caugth in single loop");
    delay(1000);
    singleLoop = false;
  }
